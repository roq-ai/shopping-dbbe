...
import { getAnimations } from 'apiSdk/animations';
import { AnimationInterface } from 'interfaces/animation';
import { getProducts } from 'apiSdk/products';
import { ProductInterface } from 'interfaces/product';

import { AnimationListPage as ShowCaseList } from 'pages/animations';

function DashboardPage() {
  const { session } = useSession();

  const discountStat = useSWR<PaginatedInterface<DiscountInterface>>(
    () => `/discounts`,
    () => getDiscounts({ limit: 1, offset: 0 }),
  );

  const animationStat = useSWR<PaginatedInterface<AnimationInterface>>(
    () => `/animations`,
    () => getAnimations({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Discounts',
      result: discountStat?.data?.totalCount,
      isLoading: discountStat.isLoading,
    },
    {
      title: 'Animations',
      result: animationStat?.data?.totalCount,
      isLoading: animationStat.isLoading,
    },
    {
      title: 'Products',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`animations`} passHref legacyBehavior>
                  <Button
